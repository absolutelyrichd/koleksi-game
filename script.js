        ...
        function renderGames(gamesToRender) {
            ...
            gamesToRender.forEach(game => {
                ...
                row.innerHTML = `
                    ...
                    <td class="p-4 font-medium whitespace-nowrap">${game.title}</td>
                    <td class="p-4 whitespace-nowrap"><span class="px-2 py-1 text-xs font-semibold rounded-full ${getPlatformBadgeClasses(game.platform)}">${game.platform}</span></td>
                    <td class="p-4 text-slate-300 whitespace-nowrap">${game.location}</td>
                    <td class="p-4 text-yellow-300 whitespace-nowrap">${game.price ? formatPrice(game.price) : 'Gratis'}</td>
                    <td class="p-4 whitespace-nowrap"><span class="px-2 py-1 text-xs font-semibold rounded-full ${game.status === 'Dimainkan' ? 'bg-yellow-500/20 text-yellow-300' : game.status === 'Selesai' ? 'bg-green-500/20 text-green-300' : 'bg-gray-500/20 text-gray-300'}">${game.status}</span></td>
